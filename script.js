...
function displaySubjects() {
    ...
    if(subjectList){
        ...
        subjects.forEach((subject, index) => {
            ...
            listItem.className = 'flex justify-between items-center py-3 px-4 bg-terminal rounded-lg neon-box';
            listItem.innerHTML = `
                <span class="text-primary">${subject}</span>
                <div class="space-x-2">
                    <button data-action="edit-subject" data-index="${index}" class="text-accent hover:text-primary">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button data-action="delete-subject" data-index="${index}" class="text-red-500 hover:text-red-400">
...
function displayAssignments() {
    ...
    if(assignmentList){
        ...
        assignments.forEach((assignment, index) => {
            const listItem = document.createElement('li');
            listItem.className = 'flex justify-between items-center py-3 px-4 bg-terminal rounded-lg mb-2 neon-box';
            listItem.innerHTML = `
                <div>
                    <span class="font-medium text-primary">${subjects[assignment.subjectIndex]}</span>
                    <p class="text-accent">${assignment.name}</p>
                    ${assignment.dueDate ? `<p class="text-sm ${new Date(assignment.dueDate) < new Date() ? 'text-red-500' : 'text-gray-400'}">Due: ${assignment.dueDate}</p>` : ''}
                </div>
                <div class="space-x-2">
                    <button data-action="edit-assignment" data-index="${index}" class="text-accent hover:text-primary">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button data-action="delete-assignment" data-index="${index}" class="text-red-500 hover:text-red-400">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            `;
...
function displayAnnouncements() {
    ...
    if(announcementList){
        ...
        announcements.forEach((announcement, index) => {
            ...
            listItem.className = 'flex justify-between items-center py-3 px-4 bg-terminal rounded-lg mb-2 neon-box';
            listItem.innerHTML = `
                <p class="text-primary">${announcement}</p>
                <div class="space-x-2">
                    <button data-action="edit-announcement" data-index="${index}" class="text-accent hover:text-primary">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button data-action="delete-announcement" data-index="${index}" class="text-red-500 hover:text-red-400">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            `;
...
function displayMembers() {
    ...
    if(memberList){
        ...
        members.forEach((member, index) => {
            ...
            listItem.innerHTML = `
                <div>
                    <span class="font-medium text-primary">${member.username}</span>
                    <span class="text-sm ${member.role === 'admin' ? 'text-accent' : 'text-gray-400'}">(${member.role})</span>
                </div>
                <div class="space-x-2">
                    <button data-action="edit-member" data-index="${index}" class="text-accent hover:text-primary">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button data-action="delete-member" data-index="${index}" class="text-red-500 hover:text-red-400">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            `;
    ...
    if(groupList){
        ...
        groups.forEach((group, index) => {
            ...
            const memberNames = group.memberIndices.map(memberIndex => 
                `<span class="inline-block bg-gray-800 rounded-full px-2 py-1 text-xs text-primary mr-1">${members[memberIndex].username}</span>`
            ).join('');
            
            listItem.innerHTML = `
                <div>
                    <h4 class="font-medium text-primary">${group.name}</h4>
                    <p class="text-sm text-accent">${subjects[group.subjectIndex]}</p>
                    <div class="mt-1 flex flex-wrap">${memberNames}</div>
                </div>
                <div class="space-x-2">
                    <button data-action="edit-group" data-index="${index}" class="text-accent hover:text-primary">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button data-action="delete-group" data-index="${index}" class="text-red-500 hover:text-red-400">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            `;
